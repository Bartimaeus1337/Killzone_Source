...
public class EnemyShipController : ShipController
{
    ...
    bool InAttackRange =>
        PlayerShip != null &&
        Vector3.Distance(PlayerShip.transform.position, _transform.position) <= _attackRange;
    ...
    public float VectorDifference => PlayerShip != null ? (PlayerShip.transform.forward - _transform.forward).magnitude : 0;
    ...
    public override void OnEnable()
    {
        ...
        //SetState(EnemyShipState.Attack);
        base.OnEnable();
    }
    ...
    {
        if (ShouldRetreat) return EnemyShipState.Retreat;
        if (PlayerShip == null) return EnemyShipState.Patrol;
        if (ShouldReposition)
        {
    ...
    void SetState(EnemyShipState state)
    {
        ...
        switch (state)
        {
            ...
            case EnemyShipState.Attack:
                ...
                //Debug.Log($"Attacking: {_target.gameObject.name} Distance: {DistanceToTarget}");
                _aiShipMovementControls.SetTarget(_target);
    ...
    {
        Vector3 direction = Random.onUnitSphere; // return random direction if player is destroyed
        if (PlayerShip != null)
        {
            direction = PlayerShip.transform.position - _transform.position;
        }
