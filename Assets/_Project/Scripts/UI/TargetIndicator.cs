...
public class TargetIndicator : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        {
            //Debug.Log($"ON-SCREEN: Target '{_target.name}' at viewport {targetViewportPos}");
            DisplayOnScreenReticle(targetViewportPos);
            return;
        }

        //Debug.Log($"OFF-SCREEN: Target '{_target.name}' at viewport {targetViewportPos}");
    ...
    void DisplayOffScreenReticle(Vector3 targetViewportPos)
    {
        ...
        //Debug.Log($"Displaying offscreen indicator for target: {_target?.name}");
    }
